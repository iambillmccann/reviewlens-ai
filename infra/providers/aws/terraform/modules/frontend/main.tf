...
resource "aws_s3_bucket" "main" {
  ...
  tags = {
    Name = "${var.name_prefix}-web-bucket"
  }
}
...
resource "aws_cloudfront_distribution" "main" {
  ...
  price_class         = "PriceClass_100"  # Only North America and Europe
...
resource "aws_cloudwatch_metric_alarm" "cloudfront_4xx" {
  ...
  statistic           = "Average"
  threshold           = "10"
  alarm_description   = "Alert when CloudFront 4xx error rate exceeds 10%"
